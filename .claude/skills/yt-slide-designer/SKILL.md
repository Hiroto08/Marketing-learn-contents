...
7. **統合検証**：`python3 _tools/checks/verify_episode.py <episode_dir>` を実行し **RESULT: PASS** にする（NARRATIONS一致・STEPS密度・タイポグラフィ・ブラウザ描画を含む全7検査）。主要スライドのスクリーンショットで実寸も目視確認（図が縮んでいないか／**見出しや図が枠の上端・下端で見切れていないか**）
   - **縦オーバーフロー（上端見切れ）注意**：要素が多い図解スライド（ファネル・比較・7要素超）は、縦センタリングで内容が枠の**上に押し出されて見出しが切れる**ことがある。check 7 は上下左右のはみ出しを検出するが、**必ず該当スライドを実レンダリング（`.active`付与）でスクショし上端が切れていないか目視**する。切れていたら要素・STEPは削らず、そのスライドの `.pad`/`.pad-t` に inline で余白を詰めて直す（例 `style="gap:.3em;padding-top:5%;padding-bottom:8%;"`）→ 再検証

## 完了条件
- [ ] `verify_episode.py` が RESULT: PASS（実出力を報告に含める）
- [ ] S1/固有図解/S18のスクリーンショット確認済み（**上端・下端の見切れが無いことを含む**。要素の多い図解スライドは特に確認）
