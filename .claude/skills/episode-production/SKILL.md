...
2. ブリーフが存在しないため、**yt-producer がその場でミニブリーフを作る**：与えられたテーマから検索KW・タイトル3案・サムネ文字・主人公候補（WebSearchでタイムリーな実例を探す）を `script.md` 冒頭に記録。時事ネタは一次ソースの鮮度（3ヶ月以内目安）を優先
3. 以降はフローA（=L3）と完全に同じ工程・同じ検査。構造v2・18枚・機械検査・QA全ゲートも同一
4. 相違点は3つだけ：話数バッジは「番外編」表記／S18の接続先は「テーマが最も近いシリーズ回」（episode-briefs.mdの表から選ぶ）／主人公の重複禁止表への追記は同様に行う
5. description.md のシリーズ案内には「本編シリーズはこちら」として再生リストを載せる
