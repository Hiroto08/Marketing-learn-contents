...
**STEPSの順序＝ナレーションの話順にする（重要）。** ビルド時、make_video.py は各スライドのSTEPを
ナレーション進行に同期して発火させる（先頭STEP＝スライド冒頭、STEP数＝段落数なら段落頭に
アンカー、それ以外は音声尺に等間隔配置。各STEPは該当ナレーションの`lead`秒前に出る）。
そのため「話す順に要素を並べたSTEPS」を書けば、映像と音声が自動的に同期する。
