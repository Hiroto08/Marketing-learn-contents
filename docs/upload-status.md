# アップロード状況（AI時代のマーケティング・ラボ）

> 自動生成：2026-09-21 00:17 UTC ／ ソース＝各話 publish_manifest.json の videoId を YouTube videos.list で実照会
> ブランチにコミット済み。他セッションはこのファイルで最新の到達点を確認できる。

凡例：public=公開済 / private=非公開（公開待ち） / 削除=YouTubeに無い / 未UP=未アップロード / —=対象物なし

| EP | タイトル領域 | 本編 | 本編状態 | Shorts | Shorts状態 |
|----|----|----|----|----|----|
| EP01 | `ep01_what-is-marketing` | `z6YKR5d3_Yg` | public | `OCXc7UIPkss` `-xkRAFGzWgk` `XPbqym9b83s` | public |
| EP02 | `ep02_3c-stp-analysis` | `rRMoT16g-NA` | public | `oNxn6VyIGW4` `MTVubY3bEaU` `ATHdF689AZY` | public |
| EP03 | `ep03_persona-pitfalls` | `L_3cGEIDKVA` | public | `xtJ6vY1HnDI` `AKjhwKKq1gg` `iWhkHQbS2O4` | public |
| EP04 | `ep04_benefit-vs-feature` | `dVoMSTPfjaQ` | public | `3tgBbO9SgyM` `eQ6aZwe81eY` `gZhv3uUzOLE` | public |
| EP05 | `ep05_customer-journey` | `3Cqe84Ex1xA` | public | `C5zBAGyPhPg` `B0WRE-CfWyk` `Kmebn5wMCdM` | public |
| EP06 | `ep06_pricing-psychology` | `q7I5eP_-eR4` | public | `yIx-1wNM3nU` `9LVGLlNPCBg` `iCFnGGDAHXU` | public |
| EP07 | `ep07_4p-4c-mix` | `h4DOXLBXIe4` | public | `9UWuv-HF5oE` `2NkjvLtVUjU` `UJMGQodVN-c` | public |
| EP08 | `ep08_usp-differentiation` | `kR4cVze8Ito` | public | `OX9e4ssdxoY` `b8-oH6LUwVk` `gWePnm0hIqo` | public |
| EP09 | `ep09_cpa-ltv` | `ttAbub0eouw` | public | `6fYRo5RvjP4` `9llVEW_M6gg` `KGiIHoni0dA` | public |
| EP10 | `ep10_copywriting-5laws` | `EVkSwCcuosE` | public | `Zz_hEf8nqns` `a5jQe-2u2FM` `wyNmdxO2xEQ` | public |
| EP11 | `ep11_sns-engagement` | `VZKZsOg6sQg` | public | `bfGqXt5yi1g` `IlIJZlksD1w` `FZWP3a1tXmQ` | public |
| EP12 | `ep12_ab-testing` | `CRFcbyviN0o` | public | `UeI-AnCBmRc` `7knKhSg9A7w` `3zW17hWP8DY` | public |
| EP13 | `ep13_ai-market-research` | `FbYaIyerJ4g` | public | `yuS4CQlmSUg` `rdQSW7nVaR0` `iy4hpgL7c0g` | public |
| EP14 | `ep14_ai-content-creation` | `3C7KbVzj_t4` | public | `GhIqXanJswE` `8kR0LLjWUic` `bUKm3uK0flg` | public |
| EP15 | `ep15_ai-data-analysis` | `RWYeR8GAqGg` | private | `cRGQQu9tT7w` `xJjCi9HGkh4` `Bnf7r91FtX8` | private |
| EP16 | `ep16_ai-chatbot` | `75cAh0oOggk` | private | `Oh03uErojXo` `KX0DzopgbyA` `sSXzopIb964` | private |
| EP17 | `ep17_brand-equity` | `w2REF-0o1fc` | private | `Ae-I_qOe_uw` `mEt-LbMpCec` `riyV0MK61Oc` | private |
| EP18 | `ep18_ltv-crm-strategy` | `66yGh5qYznQ` | private | `JCv21G8JOQA` `8OwmvUpYveY` `KJvC4xXDMMI` | private |
| EP19 | `ep19_marketing-org` | `G3muSAmlt24` | private | `5EiVzcWnYcY` `Y6J1pAUvp6g` `VKXMiuSjavY` | private |
| EP20 | `ep20_growth-hack` | `O3ANMg1Fdb8` | private※YT補完 | `l3FUvHcNYEc` `JV1lzXzxx7c` `-o5X_JSyQY4` | private |
| EP21 | `ep21_hit-products` | `K_VbaZCL1zE` | private | `oM4-5LikyTo` `gySkRFXZ-bs` `5bwru3XThg4` | private |
| EP22 | `ep22_v-shaped-recovery` | `qCVqPmcoJZE` | private | `npVdcb6WkX0` `70U5I68Df1U` `BIGlUgljaWw` | private |
| EP23 | `ep23_failure-cases` | `oC1oCzRBxpA` | private | `_yrnwzZNvKs` `7GPZiMWNk1s` `l6WZ8BV_aHo` | private |
| EP24 | `ep24_future-marketer` | `fUwzpA76OUw` | private | `iI2YMYYHQVk` `OmlvDbc0CjU` `A_vq8EUYT8U` | private |

**集計**：manifest記録 96本中 public=56 / private=40 / 削除=0

## 公開順序（絶対則：飛び級厳禁）
- 現在 public 済みの最大EP＝**EP14**／次に公開してよいのは**EP15のみ**。
- ※本編は必ずEP番号の昇順で公開。先のEPが非公開UP済みでも飛ばして公開しない（詳細 docs/autonomous-operation.md §3.1）。

## 残作業
- 次に公開：上記「次に公開してよいEP」を public 化（or 次の土曜18:00 JSTに公開予約）
- private の後続回：EP順を守って順次公開
- 各Short：Studioで「関連動画」に本編を設定（API不可・docs/shorts-related-video-checklist.md）
- 未制作：なし（L3新規制作）
